...
public class AirlineServiceImpl implements AirlineService {
	...
	// by age
	@Override
	public List<Passenger> searchByTicket(int tId) {
		List<Ticket> t = new ArrayList<>();
		Ticket tick = ticket.getById(tId);
	...
	public String deleteTicket(int tId) {
		...
		if (days >= 0) {
			...
			if (hours <= 1 && days == 0) {
				...
				return "You are eligible for 50% refund";
			} else {
				return "Your are eligible for full refund";
			}
		} else {
	...
	public void deletePassenger(int tId, String passengerName) {
		passenger.deletePassenger(tId, passengerName);
